# Whirlpool Refrigerator Pan Roller

<p align="center">
  <b>If this part saved you money, consider buying me a thank you!</b><br><br>
  <a href="https://paypal.me/jdinino"><img src="https://img.shields.io/badge/PayPal-Tip_Jar-00457C?style=for-the-badge&logo=paypal&logoColor=white" alt="PayPal Tip Jar"></a>
</p>

---

3D printable replacement pan roller assembly for Whirlpool/KitchenAid/Maytag refrigerator drawers.

<h2 align="center"><a href="https://raw.githubusercontent.com/jdinino/3d-parts/main/appliances/whirlpool-refrigerator-pan-roller/W11578839.stl" download>Download the Wheel</a> | <a href="https://raw.githubusercontent.com/jdinino/3d-parts/main/appliances/whirlpool-refrigerator-pan-roller/W11573594.stl" download>Download the Axle</a></h2>
<h3 align="center"><a href="W11578839.stl">View Wheel</a> | <a href="W11573594.stl">View Axle</a> | <a href="https://jdinino.github.io/3d-parts/appliances/whirlpool-refrigerator-pan-roller/render.html">Interactive Viewer</a></h3>

<p align="center">
  <img src="https://img.shields.io/badge/Wheel-PETG/PLA-brightgreen" alt="Wheel: PETG/PLA">
  <img src="https://img.shields.io/badge/Axle-TPU-orange" alt="Axle: TPU">
  <img src="https://img.shields.io/badge/Revision-r17-blue" alt="Revision: r17">
</p>

## Compatible Part Numbers

| Part Number | Type |
|-------------|------|
| **W11578839** | Wheel |
| **W11573594** | Axle |
| PS16660568 | Axle Alternate |

## Symptoms of Failure

- Drawer sticks or drags
- Drawer tilts to one side
- Grinding noise when opening/closing
- Drawer jumps off track

## Compatible Refrigerator Models

### Whirlpool
- Side-by-side and French door refrigerators with pan drawers

### KitchenAid
- Side-by-side and French door refrigerators with pan drawers

### Maytag
- Side-by-side and French door refrigerators with pan drawers

### Kenmore / Kenmore Elite
- Whirlpool-manufactured refrigerators with pan drawers

## Specifications

### Wheel (W11578839)

| Parameter | Value |
|-----------|-------|
| **Outer Diameter** | 31 mm |
| **Height** | 8 mm |
| **Center Hole** | 9 mm |

### Axle (W11573594)

| Parameter | Value |
|-----------|-------|
| **Total Height** | 20 mm |
| **Shoulder Diameter** | 10 mm |
| **Shoulder Height** | 2 mm |
| **Shaft Diameter** | 8 mm |
| **Shaft Height** | 8 mm |
| **Legs** | 3 at 120° |
| **Leg Length** | 10 mm |

## Print Settings

### Wheel (W11578839) - PETG or PLA

| Setting | Value |
|---------|-------|
| Layer Height | 0.15-0.2 mm |
| Wall Count | 3-4 |
| Infill | 100% |
| Print Speed | 40-60 mm/s |
| Nozzle Temp | 230-250°C (PETG) / 200-220°C (PLA) |
| Bed Temp | 70-80°C (PETG) / 50-60°C (PLA) |
| Cooling | 50-100% |
| Orientation | Flat on bed |
| Supports | None required |

### Axle (W11573594) - TPU

| Setting | Value |
|---------|-------|
| Layer Height | 0.2 mm |
| Wall Count | 4 |
| Infill | 100% |
| Print Speed | 25-30 mm/s |
| Nozzle Temp | 220-240°C |
| Bed Temp | 50-60°C |
| Cooling | 100% |
| Orientation | Shoulder down |
| Supports | None required |
| Direct Drive | Recommended |

### Material Notes

| Part | Material | Status | Notes |
|------|----------|--------|-------|
| Wheel | PETG | **Recommended** | Rigid, wear-resistant, heat tolerant |
| Wheel | PLA | Acceptable | Rigid, may degrade over time |
| Axle | TPU | **Required** | Flexible legs for snap-fit |
| Axle | PETG/PLA | Not recommended | Legs will snap, no flexibility |

## Files

| File | Description |
|------|-------------|
| `W11578839.stl` | Wheel - 3D printable model |
| `W11578839.scad` | Wheel - OpenSCAD parametric source |
| `W11573594.stl` | Axle - 3D printable model |
| `W11573594.scad` | Axle - OpenSCAD parametric source |
| `pan-roller-FSD.md` | Functional Specification Document |
| `render.html` | Interactive 3D preview |

## Installation

1. Print wheel in PETG or PLA (rigid)
2. Print axle in TPU (flexible)
3. Insert axle legs through wheel's 9mm center hole
4. TPU legs compress to fit, then snap to retain wheel
5. Install assembly in refrigerator drawer track

## Revision History

| Rev | Date | Changes |
|-----|------|---------|
| r01-r16 | 2025 | Development iterations |
| **r17** | **2025** | **Final - 3 legs with rib reinforcement, solid wedge, TPU material** |

## License

[CC BY 4.0](../../LICENSE)

## Contributing

Issues and improvements welcome. Please include:
- Refrigerator model number
- Photo of fitment
- Any dimensional adjustments needed

## Keywords

`W11578839` `W11573594` `PS16660568` `whirlpool` `kitchenaid` `maytag` `kenmore` `refrigerator` `pan roller` `drawer` `wheel` `axle` `3d print` `replacement part`

---

<p align="center">
  <img src="https://api.visitorbadge.io/api/visitors?path=https%3A%2F%2Fgithub.com%2Fjdinino%2F3d-parts%2Ftree%2Fmain%2Fappliances%2Fwhirlpool-refrigerator-pan-roller&label=Views&countColor=%23263759" alt="Views" />
</p>
