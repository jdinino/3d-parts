# Ninja Dual Brew Coffee Reservoir Mesh Lid Full Specifications Document

## Overview

Breathable mesh variant of the Ninja Dual Brew coffee reservoir lid. The solid top surface is replaced with a fine honeycomb mesh pattern that allows water to evaporate while preventing debris from entering the reservoir. Derived from the original design by louspinuso.

## Lid Specifications

| Parameter | Value |
|-----------|-------|
| Overall Length | 152mm |
| Overall Width | 96mm |
| Overall Height | 17mm |
| Corner Radius | 10mm |
| Wall Thickness | 3mm |
| Mesh Thickness | 2mm |
| Rim Height | 15mm |
| Mesh Border | 5mm |
| Mesh Area | 136 × 80mm |
| Material | PETG, PLA, ASA |

## Mesh Specifications

| Parameter | Value |
|-----------|-------|
| Pattern | Honeycomb (hexagonal) |
| Hex Hole Circumradius | 1.5mm |
| Hex Hole Diameter (vertex-vertex) | 3mm |
| Hex Hole Width (flat-flat) | ~2.6mm |
| Hex Wall Parameter | 1.0mm |
| Min Wall (vertex-vertex) | ~1.3mm |
| Min Wall (flat-flat) | ~1.7mm |
| Grid Column Spacing | ~4.33mm |
| Grid Row Spacing | ~3.75mm |

### Cross Section (Side View)
```
    ┌──────────────────────────────────────────┐
    │  ░░░░░░ mesh (2mm) ░░░░░░░░░░░░░░░░░░░  │
    │  ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░  │
────┤                                          ├────
│   │                                          │   │
│   │           (open interior)                │   │
│   │                                          │   │  15mm rim
│   │                                          │   │
│   │                                          │   │
└───┘                                          └───┘
←3mm→           ←──── 146mm ────→              ←3mm→
←──────────────── 152mm ──────────────────────→
```

### Top View (Mesh Pattern)
```
┌────────────────────────────────────────────────┐
│ ┌────────────────────────────────────────────┐ │
│ │  ┌──────────────────────────────────────┐  │ │
│ │  │  ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡  │  │ │
│ │  │   ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡   │  │ │
│ │  │  ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡  │  │ │
│ │  │   ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡   │  │ │
│ │  │  ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡ ⬡  │  │ │
│ │  └──────────────────────────────────────┘  │ │
│ │                mesh border (5mm)           │ │
│ └────────────────────────────────────────────┘ │
│                  wall (3mm)                    │
└────────────────────────────────────────────────┘
```

### Design Features
- Honeycomb mesh pattern maximizes open area while maintaining structural strength
- 5mm solid border around mesh prevents edge weakness
- 3mm perimeter walls for rigid rim structure
- 15mm rim depth for secure seating on reservoir
- Rounded corners match reservoir geometry
- Revision stamp engraved in solid border zone

## Material Requirements

| Material | Status | Reason |
|----------|--------|--------|
| PETG | **Recommended** | Heat resistant, food safe, humidity tolerant |
| PLA | Acceptable | Easy to print, may soften near brewer heat |
| ASA | Acceptable | Best heat/UV resistance, requires ventilation |
| TPU | Not recommended | Too flexible for structural lid |

## Revision History

| Rev | Date | Changes |
|-----|------|---------|
| r01 | 2026-03-03 | Initial release - honeycomb mesh variant |
